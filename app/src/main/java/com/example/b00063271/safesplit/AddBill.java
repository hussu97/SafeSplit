...
//        payers.put("You", current_amount);
    ...
    private void startHomeScreen(){
        for (HashMap<String, String> i : transactions) {
            ...
            activityDB.createActivity(C.formatNumber(i.get(C.TRANSACTION_TO_ID)), "You are owed -" + i.get(C.TRANSACTION_AMOUNT) + "- from " + i.get(C.TRANSACTION_FROM), C.ACTIVITY_TYPE_NEW_TRANSACTION, new Date());
            activityDB.createActivity(C.formatNumber(i.get(C.TRANSACTION_FROM_ID)), "You owe -" +i.get(C.TRANSACTION_AMOUNT) + "- to " + i.get(C.TRANSACTION_FROM), C.ACTIVITY_TYPE_NEW_TRANSACTION, new Date());
        }
